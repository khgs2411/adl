...
If the selected adapter uses AppleScript (`ghostty-macos` or `terminal-macos`) and capture fails in Codex with a sandbox, `osascript`, automation, or connection error, rerun the same CLI command with escalation/outside-sandbox access. Then grant macOS Automation permission for `osascript` to control the target terminal if the doctor output asks for it.
